...
try:
    with sqlite3.connect(db_path) as conn:
        query = "SELECT body FROM my_existing_table"
...
def objective(trial):
    try:
        ...
        while len(remaining_outlier_indices) > 80:
            topic_centers = {}
            ...
            if len(remaining_outlier_indices) <= 80:
                break
...
def print_trial_info(trial, final_model):
...
if __name__ == '__main__':
    ...
    for idx, trial in enumerate(successful_trials):
        ...
        try:
            ...
            save_dir = rf"D:\model_results_trial_{idx}"
            ...
            df_results.to_csv(csv_path, index=False)  # 确保index=False

            print_trial_info(trial, final_model)

            logger.info(f"Results for trial {idx} saved in directory: {save_dir}")
        except Exception as e:
