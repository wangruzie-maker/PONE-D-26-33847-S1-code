...
# 绘制文档-主题分布图
def visualize_documents(model, processed_texts, visualizations_output_dir):
    os.makedirs(visualizations_output_dir, exist_ok=True)
    fig = model.visualize_documents(processed_texts)
    fig.write_html(os.path.join(visualizations_output_dir, "documents_topics.html"))
    logging.info("Documents and Topics visualization saved.")


# 主函数：聚类和评估
def main_clustering_and_evaluation(num_topics, min_topic_size, top_n_words, hdbscan_min_cluster_size,
                                   hdbscan_min_samples, umap_n_neighbors, umap_n_components):
    ...
    # 创建并配置 HDBSCAN 和 UMAP 模型
    hdbscan_model = hdbscan.HDBSCAN(min_cluster_size=hdbscan_min_cluster_size, min_samples=hdbscan_min_samples)
    umap_model = UMAP(n_neighbors=umap_n_neighbors, n_components=umap_n_components, metric='cosine')
    ...
    visualize_documents(model, processed_texts, visualizations_output_dir)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train a BERTopic model and evaluate it.")
    parser.add_argument('--num_topics', type=int, default=15, help="Number of topics to train the model with.")
    parser.add_argument('--min_topic_size', type=int, default=10, help="Minimum size for a topic.")
    parser.add_argument('--top_n_words', type=int, default=10, help="Number of top words per topic.")
    parser.add_argument('--hdbscan_min_cluster_size', type=int, default=15, help="HDBSCAN minimum cluster size.")
    parser.add_argument('--hdbscan_min_samples', type=int, default=5, help="HDBSCAN minimum samples.")
    parser.add_argument('--umap_n_neighbors', type=int, default=15, help="UMAP number of neighbors.")
    parser.add_argument('--umap_n_components', type=int, default=5, help="UMAP number of components.")

    args = parser.parse_args()

    main_clustering_and_evaluation(
